...
rule hisat2_align:
    ...
    shadow:
        config["shadow_level"]
    shell:
        "hisat2 -p {threads} -x {params.indexdir}{params.indexprefix} -q --rna-strandness FR --phred33 --dta --min-intronlen 20 --max-intronlen 500000 -1 {input.r1} -2 {input.r2} -S {output}"
...
rule stringtie_hisat2:
    ...
    shadow:
        config["shadow_level"]
    shell:
        "stringtie {input} --fr -p {threads} -o {output}"  
...
rule scallop_hisat2:
    ...
    shadow:
        config["shadow_level"]
    shell:
        "scallop --library_type second -i {input} -o {output}" 
