...
output_file += '.' + str(max_snps) + 'lociFilt.txt'
...
fineRADpainter_input = args.infile+'.fineRADpainter.' + str(max_snps) + 'lociFilt.txt'
data.to_csv(fineRADpainter_input, header=True, sep='\t', index=False)
print('FineRADpainter input file with filtered loci saved to '+args.infile+'.fineRADpainter.' + str(max_snps) + 'lociFilt.txt')
print('---')
...
filtered_missingX_file = args.infile+'.fineRADpainter.' + str(max_snps) + 'lociFilt.samples'+str(args.max_missing_data)+'missFilt.txt'
