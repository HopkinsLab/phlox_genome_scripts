...
cvar <- "SPECIES"
use_palette <- "d3"
use_ylab <- "Average nucleotide diversity (pi)"

xbreaks <- seq(0, dat[, max(POS)], by=50)
for(mdstr in dat[, unique(MD5)]){
    filt_strs <- dat[MD5 == mdstr][1, c(FILTER1, FILTER2)]
    prefix <- str_c(out_dir, "/", str_remove(basename(fn), "\\.txt$"), 
                    ".", filt_strs[1], ".", filt_strs[2])
        
    p <- ggline(dat[MD5 == mdstr], x="POS", y=yvar, plot_type="l", numeric.x.axis=TRUE, 
...
# Plot genome-wide pi
xvar <- "SPECIES"
gnm_d <- dat[, .(Y = sum(count_diffs) / sum(count_comparisons)), by=c(xvar, "FILTER1", "FILTER2")]
